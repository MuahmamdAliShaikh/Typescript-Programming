...
//Add function
const add = function(num1: number, num2: number, cb: (x: string) => void): number{  //cb: callback function passed as a perameter,after calling so it take a value string and return type is void
    cb("Hello World")
    return num1 + num2;
};
let func: (x: number, y: number, z: (x: string)=> void) => number ;
func = add;
const f = add(2, 3, (cb: string) => {
